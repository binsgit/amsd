...
static void *getModule(void *userp){
	try {
		...
		while (thisdb->Step() == SQLITE_ROW) {
			...
			char sbuf[32] = {0};
			if (dna.Size() == 8)
				snprintf(sbuf, 31, "%016" PRIx64, *(uint64_t *)dna.BlobStore->data());
...
int AMSD::Operations::status(json_t *in_data, json_t *&out_data){
	...
	pthread_create(&pdw[0], NULL, &getSummary, &thisCtx);
	pthread_create(&pdw[1], NULL, &getPool, &thisCtx);
	pthread_create(&pdw[2], NULL, &getModule, &thisCtx);
	pthread_join(pdw[2], NULL);
	pthread_create(&pdw[3], NULL, &getDevice, &thisCtx);
